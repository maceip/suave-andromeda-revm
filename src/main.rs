...
async fn main() {
    ...
    let block: EthersBlock<TxHash> =
        provider.request("eth_getBlockByNumber", [num, include_txs]).await.unwrap();
    ...
    let rpc_state_provider: ExecutionClient<HttpRpc> =
        ExecutionClient::new("http://127.0.0.1:8545", State::new(block_rx, finalized_block_rx, 1))
            .unwrap();
